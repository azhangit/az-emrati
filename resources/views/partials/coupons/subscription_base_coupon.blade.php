<div class="card-header mb-2">
    <h3 class="h6">{{ translate('Add Your Subscription Base Coupon') }}</h3>
</div>
...
<div class="product-choose-list">
    <div class="product-choose">
        <div class="form-group row">
            <label class="col-lg-3 col-from-label" for="name">{{ translate('Subscription Product') }}</label>
            <div class="col-lg-9">
                <select name="product_ids[]" class="form-control product_id aiz-selectpicker" data-live-search="true" data-selected-text-format="count" required multiple>
                    @foreach($products as $product)
                        @if($product->category_id == 4 && $product->published == 1)
