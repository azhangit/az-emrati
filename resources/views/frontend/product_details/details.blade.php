<div class="text-left">
    ...
    <h1 class="fw-bold mb-1 py-3">
        {{ $detailedProduct->getTranslation('name') }}
    </h1>
</div>
    <!--<div class="d-none row align-items-center mb-3 dada">-->
        <!-- Review -->
    ...
    .form-check-input  {
        border:1.5px solid #9c9c9c !important;
        cursor: pointer;
    }
...
padding: 6px 8px !important;
    ...
    @media (min-width: 768px) {
        .box{
          ...
          <div  id="special-options-section" >
          <div class="justify-content-evenly my-4 d-md-flex  " >
                 <div class="row col-md-6 justify-content-center" id="select-box">
            <div class="box   p-3  col-12 ">

                <input class="form-check-input radio " type="radio" name="reference" id="oneTimeOrder" value="oneTimeOrder" required>
                <label class="form-check-label fw-bold"><b>{{ translate('One Time Order') }}</b></label>

              <div class="points">
              <div class=" my-2 box-text">
                <i class="fa-solid fa-check fa-sm me-2"></i>
                <label>{{ translate('Freshly Roasted') }}</label>
              </div>
              <div class=" my-2 box-text">
                <i class="fa-solid fa-xmark fa-sm me-2"></i>
                <label>{{ translate('Free Shipping') }}</label>
              </div>
              <div class=" my-2 box-text">
                <i class="fa-solid fa-xmark fa-sm me-2"></i>
                <label>{{ translate('Cancel Anytime') }}</label>
              </div>
              <div class="  box-text">
                <i class="fa-solid fa-xmark fa-sm me-2"></i>
                <label>{{ translate('Up to 17% Savings') }}</label>
              </div>
            </div>
            </div>
            </div>
            ...
            <input class="form-check-input radio" type="radio" name="reference" id="subscribeOrder" value="subscribeOrder" required>
<label for="subscribeOrder" class="form-check-label fw-bold"><b>{{ translate('Subscribe & Save') }}</b></label>
   ...
   $array = json_decode($detailedProduct->choice_options); // decode JSON into an array
//dd($array);
// Swap elements at index 0 and 1
function reorderAttributes(array $arr) {
    $has3 = false;
    $has1 = false;
    
    // Check if both attribute_ids "3" and "1" exist
    foreach ($arr as $item) {
    ...
    if ($has3 && $has1) {
        ...
        // Add the element with attribute_id "3" first
        foreach ($arr as $item) {
            if ($item->attribute_id === "1") {
                $newArr[] = $item;
        ...
        // Add the element with attribute_id "1" next
        foreach ($arr as $item) {
            if ($item->attribute_id === "3") {
                $newArr[] = $item;
                break;
        ...
        foreach ($arr as $item) {
            if ($item->attribute_id !== "3" && $item->attribute_id !== "1") {
                $newArr[] = $item;
            }
        }
    ...
    return $arr; // Return original array if one of the attribute IDs is missing
}

$reordered = reorderAttributes($array);
...
@endphp
        ...
        <form id="option-choice-form">
            ...
            @if ($detailedProduct->digital == 0)
                ...
                @if ($detailedProduct->choice_options != null)
                    @foreach ($reordered as $key => $choice)
                                ...
                                <div class="aiz-radio-inline">
                                    @foreach ($choice->values as $key => $value)
                                        <label class="aiz-megabox pl-0  mb-0">
                                            ...
                                            <span
                                                class="aiz-megabox-elem popup-megabox  d-flex align-items-center justify-content-center py-1 px-3">
                                                {{ $value }}
                                            </span>
                                        </label>
                                    @endforeach
    ...
    @else
        ...
        <div class="mt-3 d-flex">
            @if ($detailedProduct->digital == 0)
                ...
                @else
                    <button type="button"
                        class="btn btn-primary w-50 me-3 btn-dark rounded-pill add-to-cart "
                         onclick="addToCart()">
                        <i class="las la-shopping-bag"></i> {{ translate('Add to cart') }}
                    </button>
       <button type="button"
    class="btn btn-dark w-50 rounded-pill add-to-cart"
    onclick="@auth buyNow(); @else guestBuyNow(); @endauth">
    ...
    <div class="rtl-text">
    <p >                    <?php echo $detailedProduct->getTranslation('description'); ?>
</p>
</div>
    <!-- Share -->
...
@endphp
<script>
    $(document).on('change', '#subscribeOrder', function() {
    window.location.href = '{{ url($navBaseUrl . '/subscription') }}';
});
function guestBuyNow() {
    if (checkAddToCartValidity()) {
        $.ajax({
            ...
            error: function(xhr) {
                console.error("Guest BuyNow error:", xhr.responseText);
            }
        });
    } else {
