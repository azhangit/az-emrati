        ...
        <div class="container">
            <div class="row">
                <div class="col-xxl-8 col-xl-10 mx-auto">
                        ...
                        <form class="form-default" action="{{ route('checkout.store_delivery_info') }}" role="form" method="POST">
                            ...
                            @if($forcePickup)
                                <div class="alert alert-info mb-4">
                                    {{ translate('Local Pickup has been selected for this order. Please choose your pickup point below.') }}
                                </div>
                            @endif
                    ...
                    <style>
                        #pickupPointBtn.dropdown-toggle::after {
    display: none !important;
}
#pickupPointBtn {
    text-align: left !important;          /* For normal text */
    align-items: flex-start !important;    /* For flexbox children */
    justify-content: flex-start !important;/* For flexbox direction */
                                        ...
                                        @endif
                                        @else
                                            <input type="hidden" name="shipping_type_{{ get_admin()->id }}" value="pickup_point">
                                            <div class="row pt-3">
                                                <div class="col-12">
                                                    ...
                                                    >
                                                        ...
                                                        @foreach ($pickup_point_list as $pick_up_point)
                                                            <option value="{{ $pick_up_point->id }}">
                                                                {{ $pick_up_point->getTranslation('name') }} - {{ $pick_up_point->getTranslation('address') }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            ...
                                            @if ($physical)
                                                ...
                                                <div class="row pt-3">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <div class="mt-4 pickup_point_id_{{ $key }} d-none"></div>
                                                    </div>
                                                </div>
                                                ...
                                                @else
                                                    <input type="hidden" name="shipping_type_{{ $key }}" value="pickup_point">
                                                    <div class="row pt-3">
                                                        <div class="col-12">
                                                            <h6 class="fs-14 fw-700 mt-3">{{ translate('Local Pickup Only') }}</h6>
                                                            <p class="text-muted mb-0">{{ translate('Home delivery and carrier options are disabled for guest pickup orders.') }}</p>
...
@section('script')
    ...
    // On option click
    $('.pickup-point-option').on('click', function(e){
        e.preventDefault();
        ...
        $('#pickupPointBtn').html(
            ...
            `<span class="d-block fs-16 fw-600 mb-1">${name}</span>
             <span class="d-block opacity-50 fs-12"><i class="las la-map-marker"></i> ${address}</span>
             <span class="d-block opacity-50 fs-12"><i class="las la-phone"></i> ${phone}</span>`
        );

        // Set hidden value for form
        $('#pickup_point_id_hidden').val(id);
    });
});
