<div class="modal-body px-4 py-5 c-scrollbar-light">
    ...
    <!-- Product/Course Info -->
    @php
        $itemType = $item_type ?? ($cart->item_type ?? 'product');
        $isCourse = $itemType === 'course';
    @endphp
    <div class="media mb-1">
        @if ($isCourse && isset($course))
            <img src="{{ static_asset('assets/img/placeholder.jpg') }}" 
                data-src="{{ $course->image ? uploaded_asset($course->image) : static_asset('assets/img/placeholder.jpg') }}"
            ...
            <div class="media-body mt-2 text-left d-flex flex-column justify-content-between">
                <h6 class="fs-14 fw-700 text-truncate-2">
                    ...
                    @if ($cart->courseSchedule)
                        - {{ $cart->courseSchedule->course_level ?? '' }}
                    @endif
                </h6>
                @php
                    $variationData = $cart->variation ? json_decode($cart->variation, true) : [];
                @endphp
                @if (isset($variationData['selected_date']))
                    <small class="text-secondary">{{ translate('Date') }}: {{ $variationData['selected_date'] }}</small>
                @endif
                @if (isset($variationData['selected_time']))
                    <small class="text-secondary">{{ translate('Time') }}: {{ $variationData['selected_time'] }}</small>
                @endif
                <div class="row mt-2">
    ...
    <!-- Related product (only for products, not courses) -->
    @if (!$isCourse && isset($product))
