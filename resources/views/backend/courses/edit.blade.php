...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="align-items-center">
		<h1 class="h3">{{translate('Edit Course')}}</h1>
	</div>
</div>
			...
			<div class="card-body">
				<form action="{{ route('courses.update', $course->id) }}" method="POST" id="courseForm">
					...
					<div class="form-group mb-3">
						...
						<select class="form-control aiz-selectpicker" name="institute_id" id="institute_id" data-live-search="true" required>
							...
							@foreach($institutes as $institute)
								<option value="{{ $institute->id }}" {{ $course->institute_id == $institute->id ? 'selected' : '' }}>{{ $institute->name }}</option>
							@endforeach
						</select>
					</div>
					<div class="form-group mb-3">
						<label for="course_module">{{translate('Course Module')}} <span class="text-danger">*</span></label>
						<input type="text" placeholder="{{translate('Course Module')}}" name="course_module" class="form-control" value="{{ $course->course_module }}" required>
					</div>
					<div class="form-group mb-3">
						<label for="image">{{translate('Course Image')}}</label>
						<div class="input-group" data-toggle="aizuploader" data-type="image" data-selected="{{ $course->image }}">
							<div class="input-group-prepend">
								<div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse')}}</div>
							</div>
							<div class="form-control file-amount">{{ translate('Choose File') }}</div>
							<input type="hidden" name="image" class="selected-files" value="{{ $course->image }}">
						</div>
					...
					<div class="form-group mb-3">
						...
						<textarea class="aiz-text-editor" name="description" placeholder="{{translate('Course Description')}}">{{ $course->description }}</textarea>
					</div>
						...
						<div class="schedule-item border rounded p-3 mb-3 bg-light" data-index="{{ $index }}">
							<div class="d-flex justify-content-between align-items-start mb-3">
								<h6 class="mb-0 text-primary">
									<i class="las la-calendar-alt"></i> {{translate('Schedule')}} #{{ $index + 1 }}
								</h6>
								<div class="d-flex gap-2">
									<div class="form-group mb-0" style="width: 120px;">
										<label class="form-label small fw-bold">{{translate('Status')}}</label>
										<select name="schedules[{{ $index }}][is_available]" class="form-control form-control-sm">
											<option value="1" {{ $schedule->is_available ? 'selected' : '' }}>{{translate('Available')}}</option>
											<option value="0" {{ !$schedule->is_available ? 'selected' : '' }}>{{translate('Unavailable')}}</option>
										</select>
									</div>
									<button type="button" class="btn btn-sm btn-outline-danger remove-schedule align-self-end" {{ $course->schedules->count() <= 1 ? 'style="display:none;"' : '' }}>
							...
							<div class="row g-3">
								<div class="col-md-3">
									<div class="form-group mb-0">
										...
										<input type="date" name="schedules[{{ $index }}][date]" class="form-control form-control-sm schedule-date" value="{{ $schedule->date->format('Y-m-d') }}" required>
									</div>
								</div>
								<div class="col-md-2">
									<div class="form-group mb-0">
								...
								<div class="col-md-2">
									<div class="form-group mb-0">
										...
										<input type="text" name="schedules[{{ $index }}][course_level]" class="form-control form-control-sm" value="{{ $schedule->course_level }}" required>
									</div>
								</div>
								<div class="col-md-3">
									<div class="form-group mb-0">
										...
										<div class="input-group input-group-sm">
											...
											<input type="number" step="0.01" min="0" name="schedules[{{ $index }}][price]" class="form-control" value="{{ $schedule->price ?? $course->price }}" required>
										</div>
									</div>
								</div>
							</div>
...
@section('script')
<script>
let scheduleIndex = {{ $course->schedules->count() }};
...
document.getElementById('addSchedule').addEventListener('click', function() {
    ...
    // Remove hidden id field if exists (for new schedules)
    const hiddenId = newSchedule.querySelector('input[type="hidden"][name*="[id]"]');
    if (hiddenId) {
        hiddenId.remove();
    }
    ...
    newSchedule.querySelectorAll('input, select').forEach(input => {
        ...
        if (name) {
            input.setAttribute('name', name.replace(/\[\d+\]/, '[' + scheduleIndex + ']'));
